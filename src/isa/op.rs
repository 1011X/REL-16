	...
	fn from_str(s: &str) -> Result<Self> {
		...
		macro_rules! addr(($max:expr) => {{
			...
			// only decimal numbers currently supported
				...
				return Err(OpError::BadToken(addr_tok.to_owned()));
			}
		}});
		...
		let op = if let Ok(ra) = Reg::from_str(token) {
		    ...
		    match token {
			    ...
			    "xori" => Op::XorImm(reg!(), val!(0xFF)),
			    "addi" => Op::AddImm(reg!(), val!(0xFF)),
			    "subi" => Op::SubImm(reg!(), val!(0xFF)),
			    "roli" => Op::LRotImm(reg!(), val!(0xFF)),
			    "rori" => Op::RRotImm(reg!(), val!(0xFF)),
			    "io"   => Op::IO(reg!(), val!(0xFF)),
